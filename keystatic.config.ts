...
const pathField = (description: string) => fields.slug({
  name: { label: '文件路径', description, validation: { isRequired: true } },
  slug: {
    generate: name => name.trim(),
    description: '保留中文、空格和斜线。发布后修改路径会改变网址。',
    validation: { pattern: { regex: /^(?!.*(?:^|\/)\.{1,2}(?:\/|$))[^?#\\]+$/, message: '请使用相对路径，不含 ?、# 或反斜线。' } },
  },
});

const lines = (label: string) => fields.array(fields.text({ label: '文字' }), {
  label,
  itemLabel: props => props.value,
});

const references = (label: string, target: string) => fields.array(
...
const body = () => fields.text({
  label: '正文（Markdown）',
  multiline: true,
  description: '空行分段，单换行保留诗歌分行。图片：![](/img/图片.jpg)，下一行可写 {:.caption.bottom.right.outside caption="作者"}。排版语法见 README。',
});
...
export default config({
  ...
  collections: {
    issues: collection({
      label: '期刊', path: 'src/content/issues/*', slugField: 'path', format: 'yaml',
      schema: {
    ...
    chapters: collection({
      label: '章节', path: 'src/content/chapters/**', slugField: 'path', format: 'yaml',
      schema: {
    ...
    articles: collection({
      label: '文章', path: 'src/content/articles/**', slugField: 'path', format: 'yaml',
    ...
    images: collection({
      label: '上传图片', path: 'src/content/images/*', slugField: 'name', format: 'yaml',
      schema: {
        name: fields.slug({ name: { label: '图片名称' } }),
        image: fields.image({ label: '图片', directory: 'public/img/uploads', publicPath: '/img/uploads/', validation: { isRequired: true } }),
  ...
  singletons: {
    journal: singleton({
      label: '网站与期刊顺序', path: 'src/content/settings/journal', format: 'yaml',
      schema: {
    ...
    home: singleton({
      label: '首页', path: 'src/content/settings/home', format: 'yaml',
      schema: {
        quote: fields.text({ label: '引言', multiline: true }),
        readLabel: fields.text({ label: '阅读链接文字' }),
        issue: fields.relationship({ label: '推荐期刊', collection: 'issues', validation: { isRequired: true } }),
      },
    }),
    about: singleton({
      label: '关于', path: 'src/content/settings/about', format: 'yaml',
